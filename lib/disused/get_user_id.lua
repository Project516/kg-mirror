...
local function get_user_id_ajax(username)
    ...
    local request_body = "route_urls[0]=%2F" .. username ..
                             "&__d=www&__a=1&__comet_req=7"

    local id_request, err = httpc:request_uri(
    ...
    if not id_request then ngx.log(ngx.ERR, "Request failed: " .. err) end

    local payload = json.decode(id_request.body:sub(10, -1))

    return payload.payload.payloads["/" .. username].result.exports.hostableView
               .props.page_logging.params.profile_id
end
